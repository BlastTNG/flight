...
double az_gcirc( double *az, double *el, double el_in )
{
  double azdist;
  double u[2], v[2], v_in;
  ...
  if( fabs(azdist) > 180 )
  {
    if( az[0] < az[1] ) azdist = -(360-fabs(azdist));
    else azdist = (360-fabs(azdist)); 
  }

  // change of variable
  u[0] = az[0]*M_PI/180.;           u[1] = az[1]*M_PI/180.;
  v[0] = M_PI/2. - el[0]*M_PI/180.; v[1] = M_PI/2. - el[1]*M_PI/180.;
  ...
  if( ((el[0] > 0) && (el[1] < 0)) ||
      ((el[0] < 0) && (el[1] > 0)) ) cross_eq = 1;


  if( (azdist <= 0) && (el[0] <= el[1]) )
  {  
    if( cross_eq ) return az_2a;
    else return az_1a;
  }

  if( (azdist <= 0) && (el[0] > el[1]) )
  {
    if( cross_eq ) return az_2b;
    else return az_1b;
  }
