...
#define RSC_COMM_DEBUG 0
#if RSC_COMM_DEBUG
#include <iostream>
#endif

#define RSC_COMM_BUF_SIZE 255
...
*/
RSCCommunicator::RSCCommunicator()
{
  Init();
}
...
*/
int RSCCommunicator::openHost(string target)
{
...
*/
int RSCCommunicator::openClient(string target)
{
  EthernetRSC = 3; /* Unknown state */
  if (commFD >= 0) return -1;   //already an open connection
...
*/
void RSCCommunicator::closeConnection()
{
  if (serverFD >= 0) {
    close(serverFD);
    serverFD = -1;
...
*/
string RSCCommunicator::repairLink()
{
  fd_set test;
  timeval timeout;
  char buf[RSC_COMM_BUF_SIZE];
  int n;
  FD_ZERO(&test);
  FD_SET(commFD, &test);
  timeout.tv_sec = 2;
  ...
  //else n>0
  if ((n = read(commFD, buf, RSC_COMM_BUF_SIZE-1)) < 0) return "";
...
*/
void RSCCommunicator::readLoop(string (*interpretFunction)(string))
{
#if RSC_COMM_DEBUG
...
*/
string RSCCommunicator::looplessRead()
{
  fd_set input;
  char buf[RSC_COMM_BUF_SIZE];
  int n;
...
*/
string RSCCommunicator::buildReturnString(string returnStr)
{
  return ((string)"<str>" + returnStr + "</str>");
}

int RSCCommunicator::sendReturnString(string returnStr)
{
...
*/
int RSCCommunicator::sendCommand(string cmd)
{
  pthread_mutex_lock(&rscmutex);
#if RSC_COMM_DEBUG
  ...
  pthread_mutex_unlock(&rscmutex);
  return n;
}

int RSCCommunicator::sendCommand(const char* cmd)       //in case flight wants to use C only
{
  string str_cmd = cmd;
