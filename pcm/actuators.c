...
static void WriteDR()
{
  ...
  fp = open("/data/etc/spider/act.dr", O_WRONLY|O_CREAT|O_TRUNC, 00666);
  if (fp < 0) {
...
void ReadDR()
{
  ...
  if ((fp = open("/data/etc/spider/act.dr", O_RDONLY)) < 0) {
    read_fail = 1;
...
static void oldSetLockState(int nic)
{
  ...
  if (fabs(ACSData.enc_mean_el - LockPosition(CommandData.pointing_mode.Y)) <= 0.5)
    state |= LS_EL_OK;
