...
*/
  ...
  port = 1;
  printf("Power cycling the camera...\n");
  printf("Lowering pins on the DIO...\n ");
  ...
  Sleep(500);
  printf("Raising Data 1 on the DIO...\n ");//camera on/off
  ...
  if( QCam_ListCameras( camlist, &listlen ) != qerrSuccess) {
    QCam_ReleaseDriver();
  ...
  // So we just power cycle it again, also to make sure that any previous settings are reset.
 /*  if (listlen == 0) { //Removed support for parrallel port
    QCam_ReleaseDriver();
 ...
 if (listlen == 0) {
    QCam_ReleaseDriver();
    printf("Power cycling the DIO, again...\n");
    printf("Lowering pins on the DIO... ");
    if ((dscbresult = dscDIOOutputByte(dscb, port, 0x0)) != DE_NONE)
    {
        dscGetLastError(&errparams);
    ...
    listlen = sizeof(camlist)/sizeof(camlist[0]); // Re-inizializing listlen is needed
  ...
  if( (listlen > 0) && (camlist[0].isOpen == false) ) {
	QCam_OpenCamera( camlist[0].cameraId, &camhandle );
...
/* NNG for testing. also change pixel scale from 6.98 to 9.44, and max blobs from 7 to 5
	x[0]=930.157011-1530/2;
	x[1]=1472.587142-1530/2;
	x[2]=908.98317-1530/2;
	x[3]=1129.466186-1530/2;
	x[4]=330.391564-1530/2;
	y[0]=167.15845-1020/2;
	y[1]=922.480301-1020/2;
	y[2]=323.056624-1020/2;
	y[3]=677.975356-1020/2;
	y[4]=825.453327-1020/2;
*/   
    ...
    if( NO_CALC_POINTING ) nmatch = 0;
      ...
      nmatch = calc_pointing( ra_0_guess, dec_0_guess, lost,//3.04469,1.21351,lost,//
                              epoch, lat, lst, //39.952, 0.0,//
                              x, y, f, nMatchBlobs, 0, 
          ...
          if( thRecvParam[i] > 0) {
            ...
            if( thRecvParam[i] == sizeof(ISCStatusStruct) ) 
              ...
              // whole frame arrived
              {
                ...
                //printf("*** %x %x %x %x\n",last_cmd.data[0],last_cmd.data[1],last_cmd.data[2],last_cmd.data[3]);
...
DWORD WINAPI send_frame( LPVOID parameter ) {
  ...
  //NNG:
  //printf("Sent framenum=%i, nblobs=%i, ra=%lf, dec=%lf, sigma=%lf\n",server_data.framenum, server_data.n_blobs,
	//	server_data.ra, server_data.dec, server_data.sigma);
...
DWORD WINAPI command_exec( LPVOID parameter ) {
  ...
  if( (focusPosition != 0) && (focusPosition != -1) 
	  && ((current_focus + focusPosition) > 0) 
	  && ((current_focus + focusPosition) < FOCUS_RANGE)) 
...
int update_command( int abort ) {
  ...
  if( thCmdState == 0 ) {
    ...
    if( newCmd != -1 ) {
//NNG       printf("Starting command from port %i\n",SERVER_PORTS[newCmd]);
      if( LOUD ) {
        time_stamp( &timebuf[0], 255 ); 
      ...
      server_data.flag = 0;
      t7 = clock();
 //NNG     printf("****** Elapsed time (between handshakes): %lfs ******\n",
 //NNG            (double)(t7-t6)/CLOCKS_PER_SEC);
      t6 = t7;
      //printf(" --------> Send handshake\n");
      timeout_send();
      //printf(" --------| Sent!\n");
    ...
    //trigger mode if 10s have elapsed
             ...
             ( (triggertype == 0 ) || ( ((now-lastCmdRec) > CLIENT_TIMEOUT) 
                                        && (triggertype != 0) ) ) ) {
      //printf("*** Client hasn't sent a packet in more than %i seconds, starting exposure...\n",CLIENT_TIMEOUT); 
	  //printf("P=%ith=%i trig=%i tm=%i\n",pause, thCmdState, triggertype, now-lastCmdRec);
...
BYTE boardtype;
  if( dscInit( DSC_VERSION ) != DE_NONE )
	{
		dscGetLastError(&errparams);
		fprintf( stderr, "Error initializing driver %s\n", errparams.errstring );
		Sleep(300);
		return 0;
	}
  ...
  printf("initialized driver...\n");
	...
	if ((dscbresult = dscInitBoard(DSC_DMM, &dsccb, &dscb)) != DE_NONE)
		{
        dscGetLastError(&errparams);
        fprintf(stderr, "Talking to DMM-XT failed: %s (%s)\n", dscGetErrorString(dscbresult), errparams.errstring);
        return dscbresult;
        }
printf("DAQ is a-go \n");
  ...
  if( NO_CAMERA == 0 ) {
    ...
    QCam_GetSerialString(camhandle, SerialNo, buffersize);
  ...
  // Shut down WinIO
   //ShutdownWinIo(); 
  // Shut Down the DMM-XT DAQ
  if ((dscbresult = dscFreeBoard(dscb)) != DE_NONE)
		return dscbresult;

  if ((dscbresult = dscFree()) != DE_NONE)
		return dscbresult;
