...
/*
std::string Radian::asQString() const {
  return std::string::number( asDouble() );
}

std::string Degree::asQString() const {
  return std::string::number( asDouble() );
}

std::string Hour::asQString() const {
  std::string s;
  
  int hour( val );
  if (hour < 10) s = "0"; // Show leading zero
  s += std::string::number( hour ) +  ":";
  
  int minute = int( 60*(val - hour) );
  if (minute < 10) s+= "0";
  s += std::string::number( minute ) +  ":";
  
  double second = 3600*(val - hour - minute/60.);
  if (second < 10) s+= "0";
  s += std::string::number( second );
  
  return s;
}

std::string DMS::asQString() const {
  std::string s;
