...
/************************************************************************/
double getlst(time_t t, double lon, double jd) {
  double S, T, T0, UT;
  double gst, lst;
  struct tm *gmt;
  ...
  jd = floor(jd) - 0.5;
  gmt = gmtime(&t);
  UT = (double)gmt->tm_hour + (double)gmt->tm_min/60.0 +
...
/************************************************************************/
/* converts ra, dec into az, el; all angles in degrees                  */
/************************************************************************/
...
/****************************************************************/
/*converts az, el to ra, dec:   all angles in degrees.          */
/****************************************************************/
void azel2radec(double az, double el,time_t lst,double lat, double *ra,
                double *dec){
  double h; /*hour angle*/
  double sin_az,cos_az,sin_el,cos_el,sin_lat,cos_lat,sin_dec;
  ...
  sin_dec=-cos_az*cos_el*cos_lat+sin_el*sin_lat;
  *dec=asin(sin_dec);

  *dec *= (180.0/M_PI);
  h=atan2(sin_az*cos_el,cos_az*cos_el*sin_lat+sin_el*cos_lat);


  h *= 180.0l/M_PI;
  while (h<0.0) h+=360.0l;
  /* convert h to ra */

  *ra=(double)lst*(15.0/3600.0) - h;
  while(*ra <0.0) *ra+=360.0l;
  while(*ra >360.0) *ra-=360.0l;
}

/*********************************************************************/
/*get lst time and jdate from current mcp time                       */
/*********************************************************************/
void GetTime(time_t *lst){
  struct tm *currtime;
  double jdate;

  currtime = gmtime(&localPointingData.t);
  jdate = GetJulian(currtime);
  
  //note**** for lon deg -west, +east
  *lst = getlst(localPointingData.t, -localPointingData.lon, jdate);
  
}


/*********************************************************************/
/*determine closest star given gond. az, el and starlist.dat         */
/*********************************************************************/
void GetStar(){
  double gond_ra, gond_dec; 
  FILE *file_ptr;
  struct stype 
  {
    double RA, DEC, mag;
  };
  struct stype star[MAX_STAR];
  int i, low, middle, high;
  static int firsttime = 1; 
  double ra_soln = 0, dec_soln = 0, az_soln = 0, el_soln = 0;
  double col_from_centre, row_from_centre;
  int star_ided = 0;
  double I_F_az, I_F_el;
  time_t lst;

  //open starlist and read values into star structure 
  if (firsttime) {
    firsttime = 0;
    file_ptr = fopen(OFILE, "r+");
    if (file_ptr == NULL) {
      perror("unable to open starlist.dat");
      exit(1);
    }
    for (i=0; i< MAX_STAR; i++) {
      fscanf(file_ptr, "%lf %lf %lf\n",
             &(star[i].RA), &(star[i].DEC), &(star[i].mag));
    }
    fclose(file_ptr);
  }  

  I_F_az = localPointingData.az;
  I_F_el = localPointingData.el;

  //get lst_then when frame was grabbed 
  GetTime(&lst);

  //convert gond az, el into ra and dec
  azel2radec(I_F_az, I_F_el, lst, localPointingData.lat,
             &gond_ra, &gond_dec);
  gond_ra *= M_PI/180;
  gond_dec *= M_PI/180;

  /****search here should be DEC first then RA***/
  /*****starlist.dat needs to be sort by dec (currently RA sorted) **/

  //do a binary search of starlist in order to determine the closest star
  //search through RA's
  low = 0;
  high = MAX_STAR - 1;
  while(1){
    middle = (low + high)/2;
    if ( (gond_ra >= (star[middle].RA - ERRORBOX) )
         &&(gond_ra <= (star[middle].RA + ERRORBOX) ) ){
      break;
    }
    if (gond_ra < star[middle].RA){
      high = middle;
    } else {
      low = middle; 
    }
  ...
  //search through DEC's
  for (i = -2; i <= 2; i++){
    if( (gond_dec >= (star[middle+i].DEC - ERRORBOX) )
        &&(gond_dec <= (star[middle+i].DEC + ERRORBOX) ) ){
      star_ided += 1;
      ra_soln = star[middle+i].RA;
      dec_soln = star[middle+i].DEC;
    }
  }


  /** need to determine ARCSEC_PER_COL/ROW for this camera and lens--values used**/
  /**are old boomerang values and are approx. **/
  
  if (star_ided == 1) {
    radec2azel(ra_soln, dec_soln, lst, localPointingData.lat,
               &az_soln, &el_soln);
    col_from_centre = column_j - WIDTH/2.0;
    row_from_centre = -row_i + HEIGHT/2.0;
    az_soln += col_from_centre*ARCSEC_PER_COL/3600.0;
    el_soln += row_from_centre*ARCSEC_PER_ROW/3600.0;
    VSCData[vsc_index].az = az_soln;
    VSCData[vsc_index].el = el_soln;
    VSCData[vsc_index].mcp_time = localPointingData.t;
    VSCData[vsc_index].id_frame = VSCData[vsc_index].sf_frame;
  }
}
