...
#define EZ_RESP_ERR_MASK     0x0600    // EZ_ERR_TIMEOUT and EZ_ERR_TTY
...
struct ezbus {
  ...
  int err_count;		        // number of errors since we last successfully communicated
                                // with ezbus
  int sequence;                 // Sequential numbering of commands
};
...
char* __attribute__((format(printf, 5, 6))) EZBus_StrComm(struct ezbus* bus,
    uint8_t who, size_t len, char* buffer, const char* fmt, ...);
