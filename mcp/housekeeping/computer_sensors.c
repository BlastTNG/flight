...
double sensors_temp_mb1[2] = {0.0, 0.0};
double sensors_temp_mb2[2] = {0.0, 0.0};
double sensors_temp_cpu[2] = {0.0, 0.0};
...
void blast_store_cpu_health(void)
{
    ...
    if((err = sensors_get_value(chip[1], CH_TEMP_CPU, &sensors_temp_cpu[SouthIAm])))
    {
        blast_err("Could not get temp CPU %s", sensors_strerror(err));
    }

    if((err = sensors_get_value(chip[0], CH_TEMP_MB1, &sensors_temp_mb1[SouthIAm])))
    {
        blast_err("Could not get temp MB1 %s", sensors_strerror(err));
    }

    if((err = sensors_get_value(chip[0], CH_TEMP_MB2, &sensors_temp_mb2[SouthIAm])))
    {
