...
const char *roach5_fpg = "roach5.fpg";
...
{   roach_pack_LUTs(m_roach, m_freqs, m_freqlen);
    ...
    if (roach_write_data(m_roach, "qdr0_memory", (uint8_t*)m_roach->LUT.I, m_roach->LUT.len * sizeof(uint16_t), 0, QDR_TIMEOUT) < 0) { printf("Could not write to qdr0!");}
    roach_write_data(m_roach, "qdr1_memory", (uint8_t*)m_roach->LUT.Q, m_roach->LUT.len * sizeof(uint16_t), 0, QDR_TIMEOUT);
    sleep(0.3);
...
void roach_roach_write_tones(roach_state_t *m_roach, double *m_freqs, size_t m_freqlen)
{	
