...
bool
get_frequency(enum Synthesizer synth, float *frequency)
{
    uint8_t bytes[24];
    uint8_t checksum;
    registers_t regs;
...
bool
set_frequency(enum Synthesizer synth, float frequency,
                          float chan_spacing)
{
    vco_range_t vcor;
    int32_t dbf = 1;
...
bool
set_reference(uint32_t frequency)
{
    uint8_t bytes[6];
    bytes[0] = 0x01;
...
//----------//
int32_t
get_rf_level(enum Synthesizer synth)
{
    uint8_t bytes[24];
    uint8_t checksum;
    bytes[0] = 0x80 | synth;
...
bool
set_rf_level(enum Synthesizer synth, int32_t rf_level)
{
    int32_t rfl = 0;
    switch(rf_level)
    {
...
//---------------------//
bool
get_options(enum Synthesizer synth, options_t *opts)
{
    uint8_t bytes[24];
    uint8_t checksum;
    bytes[0] = 0x80 | synth;
...
bool
set_options(enum Synthesizer synth,
...
//-----------//
bool
get_vco_range(enum Synthesizer synth, vco_range_t *vcor)
{
    uint8_t bytes[4];
    uint8_t checksum;
    bytes[0] = 0x83 | synth;
...
bool
set_vco_range(enum Synthesizer synth,
...
//------------//
bool
get_phase_lock(enum Synthesizer synth)
{
    uint8_t bytes;
    uint8_t checksum;
    bytes = 0x86 | synth;
...
//-------------------//
bool
get_label(enum Synthesizer synth, char *label)
{
    uint8_t bytes[16];
    uint8_t checksum;
    bytes[0] = 0x82 | synth;
...
bool
set_label(enum Synthesizer synth, const char *label)
{
    uint8_t bytes[18];
    bytes[0] = 0x02 | synth;
...
//------------------//
float
getEPDF(enum Synthesizer synth)
{
    options_t opts;
