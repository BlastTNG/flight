...
int step_motor( int motor, int steps ) {
  ...
  if( motor == FOCUS_MOTOR ) {
    //position = focusPosition + steps;
    //if( position < 0 ) position = 0;
    //if( position > FOCUS_RANGE ) position = FOCUS_RANGE;
    //actualSteps = position - focusPosition;
    //focusPosition = position;
    //printf("New focus motor position will be: %i\n",focusPosition);
    
    actualSteps = steps; //focusPosition;  // KLUDGE
  }
  ...
  sprintf(cmd,"%c%i",direction,abs(actualSteps));
  ...
  if( actualSteps != 0 ) { // prevent motor spinning indefinitely
    ...
    if( (logfile=fopen(stepperlogname,"w")) == NULL ) {
      ...
      fprintf(logfile,"%i %i",focusPosition,aperturePosition);
      fclose(logfile);
    }
  }
...
void calibrate_motors( void ) {
  ...
  // Get position from the log file
    ...
    // Set focusPosition and aperturePosition to represent the logged positions
    // if valid.
    if( (logFocus>=0) && (logFocus<=FOCUS_RANGE) && (logAperture>=0) && 
        (logAperture<=AP_RANGE) ) {
      focusPosition = logFocus;
...
DWORD WINAPI command_exec( LPVOID parameter ) {
  ...
  //if( (execCmd.focus_pos<0) || (execCmd.focus_pos>FOCUS_RANGE) )
  //execCmd.focus_pos = focusPosition; // changed interpretation of
  //focus_pos
  ...
  if( (execCmd.hold_current<0) || (execCmd.hold_current>50) ) 
