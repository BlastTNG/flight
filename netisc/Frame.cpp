...
void Frame::addBlob( const Degree& x, const Degree& y, double flux) {
  ...
  qDebug( "Adding Blob( %lg, %lg)", x, y);
  blob.append( Blob(x, y, mag) );
}
...
QValueList<Star> Frame::getStars() {
  Q_ASSERT( primaryBlob );
  Q_ASSERT( secondaryBlob );
