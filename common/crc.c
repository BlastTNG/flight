 ...
 */
static inline uint32_t __pure crc32_le_generic(uint32_t crc, unsigned char const *p,
                      size_t len, const uint32_t (*tab)[256],
                      uint32_t polynomial)
{
    crc = (uint32_t) htole32(crc);
    crc = crc32_body(crc, p, len, tab);
    crc = le32toh((uint32_t)crc);
    return crc;
}
uint32_t __pure crc32_le(uint32_t crc, unsigned char const *p, size_t len)
{
 ...
 */
static inline uint32_t __pure crc32_be_generic(uint32_t crc, unsigned char const *p,
                      size_t len, const uint32_t (*tab)[256],
                      uint32_t polynomial)
{
...
static int crc32_combine_test(void)
{
    int i, j;
    int errors = 0, runs = 0;
    ...
    for (i = 0; i < 10; i++) {
        ...
        for (j = 0; j <= test[i].length; ++j) {
            uint32_t crc1, crc2;
            uint32_t len1 = j, len2 = test[i].length - j;
