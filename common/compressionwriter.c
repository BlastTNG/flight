...
unsigned short *PopFrameBufferAndSlow(struct frameBuffer *buffer,
        unsigned short ***slow); // mcp.c
...
//  Because fd = 0 through "fd = open(tty, O_RDWR | O_NOCTTY))"
    ...
    if ((fd = open(tty, O_RDWR | O_NOCTTY)) < 0) {
        if (report_state != 0) {
            bprintf(err,
                    "Compressionwriter.c: Could not open downlink serial port %s.  Retrying...",
                    tty);
            report_state = 0;
        }
        return (fd);
    ...
    if (tcgetattr(fd, &term)) {
        berror(tfatal,
                "Compressionwriter.c: Unable to get downlink serial port %s attributes",
                tty);
    }
    ...
    if (cfsetospeed(&term, B115200))
        berror(tfatal, "Error setting downlink serial port %s output speed",
                tty);

    if (cfsetispeed(&term, B115200))
        berror(tfatal, "Error setting downlink serial port %s input speed",
                tty);
        ...
        if (streamData[i_field].isFast) {
            ...
            if (streamData[i_field].mask) { // wide slow
                xu = (unsigned int) current_slow_data[i][c]
                        + ((unsigned int) current_slow_data[i][c + 1] << 16);
            } else { // narrow slow
                xu = (unsigned int) current_slow_data[i][c];
            }
        }
...
void WriteSuperFrame(unsigned short *frame)
{
        ...
        if (frameNiosList[i_field]->fast) {
            if (frameNiosList[i_field]->wide) {
                ...
                x = (unsigned int) frame[frameBi0List[i_field]->channel]
                        + ((unsigned int) frame[frameBi0List[i_field]->channel
                                + 1] << 16);
            } else {
                isWide = 0;
            ...
            if (frameNiosList[i_field]->wide) {
                isWide = 1;
                x =
                        (unsigned int) current_slow_data[frameBi0List[i_field]->index][frameBi0List[i_field]->channel]
                                + ((unsigned int) current_slow_data[frameBi0List[i_field]->index][frameBi0List[i_field]->channel
                                        + 1] << 16);
            } else {
                isWide = 0;
                x =
                        current_slow_data[frameBi0List[i_field]->index][frameBi0List[i_field]->channel];
            }
        }
    ...
    if (reset_rates) {
        ...
        higain_bytes_per_streamframe = (highgain_rate * FASTFRAME_PER_SUPERFRAME
                / (8 * SR)-frame_bytes_written) / STREAMFRAME_PER_SUPERFRAME;
        omni1_bytes_per_streamframe = (omni1_rate * FASTFRAME_PER_SUPERFRAME
                / (8 * SR)-frame_bytes_written) / STREAMFRAME_PER_SUPERFRAME;
        omni2_bytes_per_streamframe = (omni2_rate * FASTFRAME_PER_SUPERFRAME
                / (8 * SR)-frame_bytes_written) / STREAMFRAME_PER_SUPERFRAME;
        ...
        for (i_field = 0; i_field < n_streamlist; i_field++) {
            double delta = 0;
            delta =
                    streamList[CommandData.channelset_oth][i_field].samples_per_frame
                            * streamList[CommandData.channelset_oth][i_field].bits
                            / 8;
            ...
            if (higain_requested_bytes_per_streamframe + delta
                    > higain_bytes_per_streamframe) {
            ...
            if (omni1_requested_bytes_per_streamframe + delta
                    > omni1_bytes_per_streamframe) {
                if (n_omni1_stream == 0) {
            ...
            if (omni2_requested_bytes_per_streamframe + delta
                    > omni2_bytes_per_streamframe) {
                if (n_omni2_stream == 0) {
        ...
        bprintf(info,
                "High gain: %u stream fields use %.0f out of %d bytes per stream frame (%.0f free)",
                n_higain_stream, higain_requested_bytes_per_streamframe,
                higain_bytes_per_streamframe,
                higain_bytes_per_streamframe
                        - higain_requested_bytes_per_streamframe);
        bprintf(info,
                "Omni1: %u stream fields use %.0f out of %d bytes per stream frame (%.0f free)",
                n_omni1_stream, omni1_requested_bytes_per_streamframe,
                omni1_bytes_per_streamframe,
                omni1_bytes_per_streamframe
                        - omni1_requested_bytes_per_streamframe);
        bprintf(info,
                "Omni2: %u stream fields use %.0f out of %d bytes per stream frame (%.0f free)",
                n_omni2_stream, omni2_requested_bytes_per_streamframe,
                omni2_bytes_per_streamframe,
                omni2_bytes_per_streamframe
                        - omni2_requested_bytes_per_streamframe);
    }
...
void WriteStreamFrame()
{
    ...
    // MWG: Note that x either holds the value itself OR delta-x, depending on the channel.
    double x, dx;
    int xi;
    ...
    for (i_field = 0; i_field < n_streamlist; i_field++) {
        ...
        for (i_samp = 0;
            ...
            if (streamList[CommandData.channelset_oth][i_field].doDifferentiate) {
                ...
                x = dx / (double) streamData[i_field].gain; // x holds the value of delta-x
            } else {
                // apply gain
                x = (x - streamData[i_field].offset) // x holds the value of x itself
                        / (double) streamData[i_field].gain;
            }
            ...
            if (streamList[CommandData.channelset_oth][i_field].bits == 8) {
                ...
                n_streambuf++;
            } else if (streamList[CommandData.channelset_oth][i_field].bits
                    == 16) {
                if (xi > 32767)
                    xi = 32767; // truncate
                if (xi < -32767)
...
void CompressionWriterFunction(int channelset)
{
    ...
    // MWG: Determine frameList length by running through frameList until the \0 is reached.
    for (n_framelist = 0; frameList[n_framelist][0] != '\0'; n_framelist++)
        ;

    frameNiosList = (struct NiosStruct **) malloc(
            n_framelist * sizeof(struct NiosStruct *));
    frameBi0List = (struct BiPhaseStruct **) malloc(
            n_framelist * sizeof(struct BiPhaseStruct *));
    ...
    // MWG: Determine streamlist length just as with framelist length.
    bprintf(info, "Compressionwriter.c: channelset oth == %d, field 1 == %s",
            CommandData.channelset_oth, streamList[1][1].name);
    for (n_streamlist = 0;
            streamList[CommandData.channelset_oth][n_streamlist].name[0] != '\0';
            n_streamlist++)

        streamData = (struct streamDataStruct *) malloc(
                n_streamlist * sizeof(struct streamDataStruct));

    for (i_field = 0; i_field < n_streamlist; i_field++) {
        /* MWG: The old code referenced bolometers; I cut that code out, but left this
         * if-statement.
         */
        ...
        streamData[i_field].residual = 0;
        streamData[i_field].gain =
                streamList[CommandData.channelset_oth][i_field].gain;
    ...
    bprintf(startup,
            "Compressionwriter.c, CompressionWriterFunction: frame list length == %d; stream list length == %d",
            n_framelist, n_streamlist);
