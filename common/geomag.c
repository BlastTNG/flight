...
GEOMAG:
  if ((wmmdat = fopen("/data/etc/WMM.COF", "r")) == NULL) {
    berror(err, "World Magentic Model: Error opening WMM.COF");
    return;
  }
  ...
  if (fgets(c_str, 80, wmmdat) == NULL) {
    berror(err, "World Magentic Model: Error while reading WMM.COF");
    return;
  }
...
GEOMG1:
  ...
  {
    bprintf(err, "World Magentic Model: !!!! Mag. model needs a year between "
        "%f and %f !!!!\n", epoch, epoch + 5.0);
    return;
  }
